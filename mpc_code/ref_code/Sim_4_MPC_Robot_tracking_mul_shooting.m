...
states = [x;y;theta]; n_s = length(states);

v = SX.sym('v'); omega = SX.sym('omega');
controls = [v;omega]; n_c = length(controls);
...
%               (con-P(5*k+2:5*k+3))'*R*(con-P(5*k+2:5*k+3)) ; % calculate obj
    ...
    % the first state is IC
    obj = obj + (st-P(n_s+(n_s+n_c)*(k-1)+1:n_s+(n_s+n_c)*(k-1)+n_s))'*Q*(st-P(n_s+(n_s+n_c)*(k-1)+1:n_s+(n_s+n_c)*(k-1)+n_s)) + ...
        (con-P(n_s+(n_s+n_c)*(k-1)+n_s+1:n_s+(n_s+n_c)*(k-1)+n_s+n_c))'*R*(con-P(n_s+(n_s+n_c)*(k-1)+n_s+1:n_s+(n_s+n_c)*(k-1)+n_s+n_c));
