...
%     solMPC.xx1(:,1:n_s,mpciter+1)= reshape(full(sol.x(1:n_s*(N+1)))',n_s,N+1)'; % get solution TRAJECTORY
    ...
    std = 0.5;
    [t0, x0, U_DEC] = shift_noise(DT, t0, x0, u,f,std);
    x_traj(:,mpciter+2) = x0;
...
% else
%     file_name = stpheight+"_"+dir+"_"+"N"+Nstart+"_"+"Trajectory.mat";
% end
file_name = stpheight+"_N"+Nstart+"_noise"+std+".mat";
save(fullfile(pwd, 'Results/ControllerNoise/', file_name));
