...
Mmat = mtimes*Mmat;
Fv = G;
Gv_u = btimes*B*u_ctrl;
Gv = Gv_u;
X_inv = Jac*(Mmat\Jac');
lambda = -X_inv \ (dJac*dq + Jac*(Mmat\(Fv + Gv)));
Gv_c = Jac'*lambda;
Gv = Gv + Gv_c;
rhs_mismatch = [dq; Mmat\(Fv + Gv)];
f_mismatch = Function('f_mismatch',{states,u_ctrl},{rhs_mismatch});
...
%     solMPC.xx1(:,1:n_s,mpciter+1)= reshape(full(sol.x(1:n_s*(N+1)))',n_s,N+1)'; % get solution TRAJECTORY
    ...
    % USING MISMATCH TO PROPOGATE ACTUAL TRAJ
    [t0, x0, U_DEC] = shift(DT, t0, x0, u,f_mismatch);
    x_traj(:,mpciter+2) = x0;
...
if Nstart == 10
    file_name = stpheight+"_"+dir+"_mass"+mtimes;
else
    file_name = stpheight+"_"+dir+"_mass"+mtimes+"_"+"N"+Nstart;
end

if btimes ~= 1
   file_name = file_name + "_B"+btimes;
end

file_name = file_name+".mat";
save(fullfile(pwd, 'Results/ModelMismatch/', file_name));
