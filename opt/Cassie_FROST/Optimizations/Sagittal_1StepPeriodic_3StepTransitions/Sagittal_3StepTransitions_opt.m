...
nlp.Phase(1).Plant.UserNlpConstraint = @Sagittal_3StepTransitions.opt.right_stance;
nlp.Phase(2).Plant.UserNlpConstraint = @Sagittal_3StepTransitions.opt.left_impact;
nlp.Phase(3).Plant.UserNlpConstraint = @Sagittal_3StepTransitions.opt.left_stance;
nlp.Phase(4).Plant.UserNlpConstraint = @Sagittal_3StepTransitions.opt.right_impact;
...
% Set initial state
Periodic = load('one_step/local/output/GaitLibrary_v1');
selected = ones(40,1);
...
addNodeConstraint(nlp.Phase(1), cassie.constraints.state_external(nlp.Phase(1)), ...
    {'x','dx'}, 'first', -1e-2, 1e-2, 'Linear', {selected, x0, dx0});

% Set final state
Periodic = load('one_step/local/output/GaitLibrary_v1');
selected = ones(40,1); selected(1:4) = 0;
x0 = Periodic.GaitLibrary{3}.states{1}.x(:,end);
dx0 = Periodic.GaitLibrary{3}.states{1}.dx(:,end);
addNodeConstraint(nlp.Phase(5), cassie.constraints.state_external(nlp.Phase(5)), ...
    {'x','dx'}, 'last', -1e-2, 1e-2, 'Linear', {selected, x0, dx0});
...
% compileConstraint(nlp.Phase(1),'state_external_constraint',PATHS.OPT_EXPORT);
